...
class ImagenGaleriaLogoNegro extends StatefulWidget {
  const ImagenGaleriaLogoNegro({
    Key? key,
    this.width,
    this.height,
  }) : super(key: key);
  ...
  final double? height;
  @override
  State<ImagenGaleriaLogoNegro> createState() => _ImagenGaleriaLogoNegroState();
}

class _ImagenGaleriaLogoNegroState extends State<ImagenGaleriaLogoNegro> {
  @override
  void initState() {
    inicioDeArchivo();
    super.initState();
  ...
  Widget build(BuildContext context) {
    return IconButton(
      ...
      icon: FFAppState().tempoImagenOscuro == null
          ? Image.file(
              archivo!,
              fit: BoxFit.fill,
            )
  ...
  Future<void> selectImage() async {
    ...
    if (selectedImagePath.isNotEmpty) {
      try {
        ...
        FFAppState().update(() {
          FFAppState().tempoImagenOscuro = selectedImagePath;
        });
        setState(() {});
      } catch (e) {
        print("Error: $e");
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
            content: Text("¡No se ha seleccionado o capturado una imagen!")),
      );
    }
  }
